...
import torch
from metrics import accuracy, coverage, independence_metrics, separation_metrics
...
def risk(y_true, y_pred, x_sens):
    ...
    corect = y_true[(y_pred != 10000.0)] == y_pred[(y_pred != 10000.0)]
...
def run_experiment(
    ...
    key = method
    BASE_MODEL_SIGNATURE = "{}_{}{}_{}".format(data_name, method, postfix, seed)
    ...
    """
    transformer = TransformDPAbstantion(clf, alphas=alphas_dict, randomize=randomize)
    transformer.fit(X_unlab)
    y_pred = transformer.predict(X_test)
    y_pred_unf = clf.predict(X_test)

    y_pred = torch.from_numpy(np.where(y_pred == 10000.0, -1, y_pred))
    y_test = torch.from_numpy(y_test)
    sens = torch.from_numpy(X_test[:, -1])

    results = (
        coverage(y_pred)
        | accuracy(y_pred, y_test)
        | independence_metrics(y_pred, sens)
        | separation_metrics(y_pred, y_test, sens)
    )
    return results
