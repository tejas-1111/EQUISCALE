"""Contains loss functions and preformance metrics"""
...
class DoubleSigmoidLoss(nn.Module):
    ...
    def forward(
        ...
        Returns:
            A 0-dimensional tensor denoting the average of the double sigmoid loss
...
class GeneralizedCrossEntropy(nn.Module):
    """
    Surrogate loss function using Generalized cross entropy for training the model
    according to the paper "Generalizing Consistent Multi-Class Classification with Rejection
    ...
    def __init__(self, cost: float, gamma: float) -> None:
        """
        Initializes surrogate loss function with GCE.
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x:
              Tensor of shape (batch_size, *) where the last dimension is the Likelihood
              for abstention.
            y:
        ...
        Returns:
            A 0-dimensional tensor denoting the average of the GCE over all the
...
class FairnessConstraints(nn.Module):
    ...
    def forward(self, r, true, sens) -> torch.Tensor:
        return torch.stack(
            [condition(r=r, true=true, sens=sens) for condition in self.conditions],
            dim=0,
        )
...
def coverage(pred: torch.Tensor) -> dict[str, torch.Tensor]:
    """
    Calculates the overll coverage of a model.
